...
test('### Test Save Error Case ###', testWrap(function (t) {
  // Data
...
test('### Test Save Valid Data ###', testWrap(function (t) {
  // Data
  const Model = ARROW.getModel('Posts')
  const instance = {
    toPayload: function () {
  ...
  const queryStub = this.stub(CONNECTOR, '_query').callsFake(function (query, values, cbSpy, executor) {
...
test('### Test Save Valid Data nothing to save ###', testWrap(function (t) {
  // Data
  const Model = ARROW.getModel('Posts')
  const instance = {
    toPayload: function () {
