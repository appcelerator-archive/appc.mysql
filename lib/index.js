...
exports.create = function(APIBuilder, server) {
	...
	return Connector.extend({
		...
		query: function(Model, options, callback) {
			...
			var sel = this.translateObjectForPayload(Model, options.sel),
				unsel = this.translateObjectForPayload(Model, options.unsel);
			if (sel) {
			...
			var where = this.translateObjectForPayload(Model, options.where);
			...
			var order = this.translateObjectForPayload(Model, options.order);
			if (order) {
				orderQuery = ' ORDER BY';
				for (key in  order) {
		...
		getTableName: function getTableName(Model) {
			var table = Model.getMeta('table') || Model._supermodel || Model.name;
		...
		translateObjectForPayload: function translateObjectForPayload(Model, obj) {
			if (!obj) {
				return obj;
			}
			...
			for (var fieldKey in Model.fields) {
				if (Model.fields.hasOwnProperty(fieldKey)) {
