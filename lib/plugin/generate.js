...
function generate(opts, callback) {
	...
	async.series([
		...
		function(cb) {
			async.eachSeries(tables, function(table, done){
				connection.query('desc '+table, function(err,results){
					...
					buffer+="var Model = APIBuilder.createModel('"+table.toLowerCase()+"',"+JSON.stringify(obj,'\t',4)+");\n\n";
		...
		function(cb) {
			var from = path.join(__dirname,'..','index.js'),
				to = path.join(config.dir, 'lib', 'index.js'),
				fromBuf = fs.readFileSync(from).toString(),
				toBuf = fromBuf.replace(/'appc.mysql'/g,"'"+config.name+"'");
