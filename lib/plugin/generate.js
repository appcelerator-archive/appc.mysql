...
/**
 * Generates based on the provided arguments.
 * @param appc
 * @param opts
 * @param callback
 */
		...
		function (cb) {
			async.eachSeries(tables, function (table, done) {
				connection.query('desc ' + table, function (err, results) {
					...
					buffer += 'var Model = Arrow.Model.extend(\'' + table.toLowerCase() + '\',' + JSON.stringify(obj, '\t', 4) + ');\n\n';
...
var StringTypeRE = /^(VARCHAR|NULL|STRING|VAR_STRING|CHAR|TEXT|MEDIUMTEXT|BLOB)/i,
	NumberTypeRE = /^(DECIMAL|TINY|SHORT|LONG|FLOAT|DOUBLE|NEWDECIMAL|INT24|BIGINT|INT|TINYINT|SMALLINT|MEDIUMINT|BIT)/i,
	DateTypeRE = /^(TIMESTAMP|DATE|TIME|DATETIME|YEAR|NEWDATE)/i,
	ArrayTypeRE = /^(ENUM|SET)/i,
	LengthRE = /\(([\d]+)\)/,
	EnumRE = /\((['\w\s,]+)\)/;

/**
 * Sets the field type based on the parsed row type.
 * @param row
 * @param field
 */
...
/**
 * Sets the field length based on the provided type's parsed length.
 * @param type
 * @param field
 */
